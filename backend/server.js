...
async function generateSummaryForMrn(mrn) {
  ...
  // Read from env (ABACUS_MODEL=claude-opus-4-6)
  ...
  // 3) Group content by visit (note)
  const visits = new Map(); // note_id -> { created_date, lines[] }
  ...
  for (const r of rows) {
    ...
    // Keep component as source context ONLY (output will be a single paragraph with no headings/labels)
    visits.get(r.note_id).lines.push(component ? `${component}: ${text}` : text);
  }

  // 4) Build visits text (LATEST FIRST)
  const visitsText = [...visits.entries()]
    .map(([noteId, v], index) => {
      const created = v.created_date ? ` | created_date: ${v.created_date}` : '';
...
`.trim();
      ...
      messages: [
        {
          role: 'system',
          content:
            'Return ONLY valid JSON matching the required schema. The "text" value must be exactly one paragraph with no headings/labels and no line breaks.'
        },
  ...
  if (!parsed || typeof parsed.text !== 'string') {
    throw new Error('AI response JSON missing required "text" field');
  }

  // Enforce "single paragraph" even if the model accidentally inserts newlines
